...
\date[EUTYPES WG'18] % (optional, should be abbreviation of conference name)
...
% \section{Parametricity, Shape-irrelevance, irrelevance}
% \begin{frame}
%     \frametitle{Parametricity, Shape-irrelevance, irrelevance}
%     4 functors on bridge/path cubical sets:
%     \begin{description}
%         \item[$\id$] Non-parametricity (continuity)
%         \item[$\sharp$] Parametricity
%         \item[$\bullet \bullet$] Shape-irrelevance
%         \item[$\bullet$] Irrelevance
%     \end{description}
%     such that $\sharp \circ (\bullet \bullet) = \bullet$.

%     \bigskip

%     Abel et al. have: $\interp{\Size} = \interp{\IN}$.

%     We have: $\interp{\Size} = \bullet \bullet \interp{\IN}$. Hence, $\sharp \interp \Size = \bullet \interp \IN$.

%     \begin{center}
%         \begin{equation*}
%             \begin{array}{r | r || r}
%                 \text{ParamDTT} & \text{Abel} \qquad \qquad {\,} & \interp{\text{domain}}\\ \hline
%                 \Pi(i : \Size).A~i & \bullet \bullet (i : \Size) \to A~i & \bullet \bullet \interp \IN \\
%                 \forall(i : \Size).A~i & \bullet (i : \Size) \to A~i & \bullet \interp \IN
%             \end{array}
%         \end{equation*}
%     \end{center}
% \end{frame}
...
%   \item Works for \emph{type assignment} NbE \\ \relax
%     [e.g., Abel/Aehlig/Coquand/Dybjer, 2007--].
%   \item Not for \emph{intrinsically typed} NbE \\ \relax
%     [e.g., Altenkirch/Kaposi, FSCD 2016].
%   \end{itemize}
% \end{frame}
...
\end{minipage}
% \hspace{0.15\textwidth}
%     \begin{beamercolorbox}[rounded=true, shadow=true, title=A,
%       wd=0.7\textwidth]{block body}
% The irrelevance modality allows us to ignore sizes where they just help the type checker.
% \[
%   \suc {{\color{grey}\mathit{ignoreMe}}} n ~:~ \Nat^{\alert{\mathit{dontIgnoreMe}}}
% \]
