...
Canonical natural numbers $c$ are constructed by $\zero a$ and $\suc a t$.  Size expression $a$ is either a constant $o \in \NN$, a variable $\ind i + o$ possibly with increment $o$, or the limit ordinal $\infty$ which stands for $\omega$.  The size expression $s$ in the constructors is a suggestion for the type checker but bears no semantic significance.  For example, in the declarative typing presented here, we can have $\der \zero 5 : \Nat 1$; in the algorithmic typing however, $\der \zero 5 \jchk \Nat 1$ will be an error.

%We write $\sigma$ and $\tau$ for substitutions, which are lists of terms, and
Substitutions $\sigma$ are simply lists of terms which provide one term as replacement for each free de Bruijn index in a term $t$. We write $t \sigma$ for the application of substitution $\sigma$ to term $t$ which is defined as usual.  We write $\slift^k_m$ for the substitution $(\ind{k+m-1},\dots,\ind{k+1},\ind k)$ which applies to a term with $m$ free indices and increases all of them by $k$.  We write $\slift_m$ for the lifting $\slift^1_m$ and $\sid_m$ for the identity substitution $ \slift^0_m$. In general, we refer to liftings by letter $\xi$.
