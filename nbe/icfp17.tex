...
\documentclass[acmlarge%,review,anonymous
...
Going beyond the standard syntax-directed equality check, there is a technique that can deal with dynamic $\eta$-expansion.  It is a type-directed normalization function inspired by normalization-by-evaluation (NbE) that computes $\eta$-long normal forms \cite{bergerSchwichtenberg:lics91,danvy:tdpe}.  We can check the computed normal forms for identity and, thus, decide definitional equality.
...
borrowed from \citet{pfenning:intextirr}. $\erpiT[\noterased] U T$ is
...
%
Similar as for $\Pi$, we use named lambda abstraction as sugar for de
...
Reflection and reification perform $\eta$-expansion so that we arrive at an $\eta$-long $\beta$-normal form.  To perform the $\eta$-expansion, not the precise type is needed, just the approximate shape, in particular, whether it is a function type (do expand) or a base type (do not expand).
...
$v_1 \nfeq v_2$.  Let $u_i = \up{A_i} \lev k$.  Note that $u_1 = u_2 \in A$ by induction hypothesis.
...
\item By transitivity of judgmental equality from (\ref{it:sound}), using
  Lemma~\ref{lem:erase}:
  $\Gamma \der t = \nbe \Gamma T t = \nbe \Gamma T {t'} = t' : T$.
...
However, the usual bidirectional construction \cite{abelScherer:types10} does not seem to go through as we lack uniqueness of types.
